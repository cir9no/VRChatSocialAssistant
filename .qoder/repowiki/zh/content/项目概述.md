...
- [memory_config.yaml](file://config/memory_config.yaml)
- [audio_capturer.py](file://src/audio_capture/audio_capturer.py)
- [device_manager.py](file://src/audio_capture/device_manager.py)
- [memory_manager.py](file://src/memory/memory_manager.py)
- [vector_database.py](file://src/memory/vector_database.py)
- [profile_store.py](file://src/memory/profile_store.py)
- [conversation_store.py](file://src/memory/conversation_store.py)
- [embedding_service.py](file://src/memory/embedding_service.py)
- [retriever.py](file://src/memory/retriever.py)
...
- [test_memory_basic.py](file://tests/test_memory_basic.py)
</cite>
...
记忆与检索模块是系统实现个性化建议的核心，通过RAG（检索增强生成）技术，将历史对话和好友信息融入当前建议生成过程。该模块由多个协同工作的子组件构成，形成完整的数据存储与检索体系。
...
+add_conversation(friend_id, transcript, speaker_id) str
+retrieve_memories(query, friend_id, top_k, strategy) Memory[]
+create_friend_profile(name, voice_profile_path) str
...
+get_profile(friend_id) FriendProfile
+get_profile_by_name(name) FriendProfile
+update_profile(friend_id, updates) bool
+delete_profile(friend_id) bool
+list_all_profiles() FriendProfile[]
+count() int
}
class ConversationStore {
+db_path
+add_conversation(conversation) str
+get_conversation(conversation_id) Conversation
+get_conversations_by_friend(friend_id, limit, start_time, end_time) Conversation[]
+get_recent_conversations(days, limit) Conversation[]
+delete_conversations_by_friend(friend_id) int
+count_by_friend(friend_id) int
+count_total() int
}
...
+_dimension
+encode(text) float[]
+batch_encode(texts, batch_size) List[]float~~
+get_dimension() int
...
+client
+collection
+add_vectors(ids, embeddings, metadatas, documents) void
+search(query_embedding, top_k, filter) SearchResult[]
...
+time_decay_lambda
+retrieve(query, friend_id, top_k, strategy, apply_time_decay) Memory[]
+_semantic_search(query, friend_id, top_k, apply_time_decay) Memory[]
+_hybrid_search(query, friend_id, top_k, apply_time_decay) Memory[]
+_convert_to_memories(search_results, apply_time_decay) Memory[]
+_calculate_time_decay(timestamp) float
}
MemoryManager --> ProfileStore : "使用"
MemoryManager --> ConversationStore : "使用"
MemoryManager --> EmbeddingService : "使用"
MemoryManager --> VectorDatabase : "使用"
MemoryManager --> Retriever : "使用"
Retriever --> VectorDatabase : "查询"
Retriever --> EmbeddingService : "编码"
Retriever --> ProfileStore : "获取姓名"
```

**图表来源**
- [memory_manager.py](file://src/memory/memory_manager.py#L30-L476)
- [profile_store.py](file://src/memory/profile_store.py#L36-L328)
- [conversation_store.py](file://src/memory/conversation_store.py#L24-L401)
- [embedding_service.py](file://src/memory/embedding_service.py#L28-L351)
- [vector_database.py](file://src/memory/vector_database.py#L21-L305)
- [retriever.py](file://src/memory/retriever.py#L26-L398)

#### 数据存储层组件

记忆与检索模块的数据存储层由三个核心组件构成，分别负责不同类型数据的持久化存储：

1.  **向量数据库 (Vector Database)**
    - **技术实现**：基于Chroma向量数据库，将对话文本转换为高维向量进行存储和索引。
    - **核心功能**：支持高效的语义相似度检索，是RAG系统的核心。通过`add_vectors`方法将对话嵌入向量存入数据库，并通过`search`方法根据查询向量快速找到最相关的记忆片段。
    - **配置文件**：`config/memory_config.yaml`中的`vector_db`部分定义了持久化目录和后端选择。
    - **文件来源**：[vector_database.py](file://src/memory/vector_database.py)

2.  **好友档案 (Friend Profile)**
    - **技术实现**：采用每个好友一个JSON文件的轻量级存储方案，存储在`data/profiles/`目录下。
    - **核心功能**：存储好友的静态和动态属性，如昵称、兴趣偏好、禁忌话题、性格标签以及声纹文件路径。`ProfileStore`类提供了CRUD操作接口。
    - **数据模型**：`models.py`中的`FriendProfile`类定义了档案的完整数据结构。
    - **文件来源**：[profile_store.py](file://src/memory/profile_store.py), [models.py](file://src/memory/models.py)

3.  **对话历史 (Conversation History)**
    - **技术实现**：使用SQLite数据库进行存储，文件位于`data/conversations/conversations.db`。
    - **核心功能**：持久化存储每一次对话的详细记录，包括时间戳、说话人、原始文本、事件类型和置信度。`ConversationStore`类提供了按好友、时间范围等条件查询的接口。
    - **数据模型**：`models.py`中的`Conversation`类定义了对话记录的数据结构。
    - **文件来源**：[conversation_store.py](file://src/memory/conversation_store.py), [models.py](file://src/memory/models.py)
...
1.  **记忆管理器 (MemoryManager)**
    - **角色**：作为整个记忆模块的统一入口和协调中心，封装了所有复杂操作。
    - **职责**：初始化所有子组件（`_initialize_components`），加载配置（`_load_config`），并对外提供高层抽象接口，如`add_conversation`、`retrieve_memories`和`create_friend_profile`。
    - **文件来源**：[memory_manager.py](file://src/memory/memory_manager.py)

2.  **文本嵌入服务 (Embedding Service)**
    - **角色**：负责将文本转换为向量表示的“翻译官”。
    - **职责**：使用`BAAI/bge-m3`模型将对话文本编码为向量。支持自动从ModelScope或HuggingFace下载模型，并提供`encode`和`batch_encode`方法。
    - **文件来源**：[embedding_service.py](file://src/memory/embedding_service.py)

3.  **检索器 (Retriever)**
    - **角色**：执行检索策略的“搜索专家”。
    - **职责**：实现多种检索策略，包括语义检索、混合检索（语义+关键词）和时间衰减检索。它协调`EmbeddingService`生成查询向量，并在`VectorDatabase`中执行搜索，最后将结果转换为`Memory`对象。
    - **文件来源**：[retriever.py](file://src/memory/retriever.py)
...
- [test_memory_basic.py](file://tests/test_memory_basic.py#L1-L100)
